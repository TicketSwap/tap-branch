# AGENTS.md - AI Agent Development Guide for tap-branch
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from Branch.

### Key Components

1. **Tap Class** (`tap_branch/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_branch/client.py`): Handles API communication and authentication
1. **Streams** (`tap_branch/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_branch/streams.py`
...
```python
class MyNewStream(BranchStream):
    name = "my_new_stream"
...
```bash
tap-branch --config config.json --discover
tap-branch --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_branch/tap.py`
...
```bash
# .env.example
TAP_BRANCH_API_URL=https://api.example.com
TAP_BRANCH_API_KEY=your_api_key_here
TAP_BRANCH_BATCH_SIZE=100  # New setting
```
...
Before filing, search existing issues to avoid duplicates. Include the SDK version (`uv run tap-branch --version`), Python version, and a minimal reproduction case when reporting bugs.
